...
export const brandTruthFacts = mysqlTable(
  "brand_truth_facts",
  {
    ...
    temporalStatus: mysqlEnum("temporalStatus", ["current", "historical", "discontinued", "unknown"]).default("unknown").notNull(),
    companyEntityChange: boolean("companyEntityChange").default(false).notNull(),
    productServiceStatus: mysqlEnum("productServiceStatus", ["current", "historical", "discontinued", "unknown"]).default("unknown").notNull(),
...
export const understandingQuestionVersions = mysqlTable("understanding_question_versions", {
  ...
  locale: varchar("locale", { length: 32 }).default("zh-CN").notNull(),
  effectiveFrom: timestamp("effectiveFrom").notNull(), effectiveTo: timestamp("effectiveTo"), createdAt: timestamp("createdAt").defaultNow().notNull(), createdBy: int("createdBy"),
...
export const understandingMethodologyDimensionWeights = mysqlTable("understanding_methodology_dimension_weights", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), methodologyVersionId: varchar("methodologyVersionId", { length: 36 }).notNull(), dimension: mysqlEnum("dimension", ["identity", "business", "capability", "boundary", "temporal", "evidence", "consistency", "uncertainty", "category", "products_services", "customers", "scenarios", "capability_differentiation", "boundary_temporal", "product_service", "target_customer", "scenario"]).notNull(), weightBasisPoints: int("weightBasisPoints").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
...
export const understandingMethodologyDimensionDefinitions = mysqlTable("understanding_methodology_dimension_definitions", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), methodologyVersionId: varchar("methodologyVersionId", { length: 36 }).notNull(),
  dimension: mysqlEnum("dimension", ["identity", "category", "business", "product_service", "target_customer", "scenario", "capability_differentiation", "boundary_temporal"]).notNull(), displayName: varchar("displayName", { length: 128 }).notNull(), weightBasisPoints: int("weightBasisPoints").notNull(),
  factKeys: json("factKeys").$type<string[]>().notNull(), questionTypes: json("questionTypes").$type<string[]>().notNull(), extractionFields: json("extractionFields").$type<string[]>().notNull(), judgmentRules: json("judgmentRules").$type<Record<string, unknown>>().notNull(), coverageThresholdBasisPoints: int("coverageThresholdBasisPoints").notNull(),
  unverifiableConditions: json("unverifiableConditions").$type<string[]>().notNull(), confidencePolicy: json("confidencePolicy").$type<Record<string, unknown>>().notNull(), severityRules: json("severityRules").$type<Record<string, unknown>>().notNull(), subdimensions: json("subdimensions").$type<string[]>().notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => ({ dimensionUnique: uniqueIndex("understanding_methodology_definition_dimension_unique").on(table.methodologyVersionId, table.dimension), versionProjectFk: foreignKey({ columns: [table.methodologyVersionId, table.projectId], foreignColumns: [understandingMethodologyVersions.id, understandingMethodologyVersions.projectId], name: "understanding_methodology_defs_version_project_fk" }) }));

export const understandingQuestionDimensionBindings = mysqlTable("understanding_question_dimension_bindings", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), questionVersionId: varchar("questionVersionId", { length: 36 }).notNull(),
  primaryDimension: mysqlEnum("primaryDimension", ["identity", "category", "business", "product_service", "target_customer", "scenario", "capability_differentiation", "boundary_temporal"]).notNull(), secondaryDimensions: json("secondaryDimensions").$type<string[]>().notNull(), subdimension: varchar("subdimension", { length: 64 }), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => ({ questionUnique: uniqueIndex("understanding_question_dimension_question_unique").on(table.questionVersionId), questionProjectFk: foreignKey({ columns: [table.questionVersionId, table.projectId], foreignColumns: [understandingQuestionVersions.id, understandingQuestionVersions.projectId], name: "understanding_question_dimension_question_project_fk" }) }));
...
export const understandingAssessmentDimensionResults = mysqlTable("understanding_assessment_dimension_results", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), assessmentId: varchar("assessmentId", { length: 36 }).notNull(), dimension: mysqlEnum("dimension", ["identity", "business", "capability", "boundary", "temporal", "evidence", "consistency", "uncertainty", "category", "products_services", "customers", "scenarios", "capability_differentiation", "boundary_temporal", "product_service", "target_customer", "scenario"]).notNull(), scoreBasisPoints: int("scoreBasisPoints"), coverageBasisPoints: int("coverageBasisPoints").notNull(), confidenceBasisPoints: int("confidenceBasisPoints").notNull(), resultPayload: json("resultPayload").$type<Record<string, unknown>>().notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
...
/** PR-04A: public Evidence Ledger. No aggregate Trust score is stored or computed. */
export const trustEvidenceSources = mysqlTable("trust_evidence_sources", {
  id: varchar("id", { length: 36 }).primaryKey(), projectId: int("projectId").notNull(), sourceUrl: varchar("sourceUrl", { length: 2000 }).notNull(), sourceOwner: varchar("sourceOwner", { length: 255 }),
  ownership: mysqlEnum("ownership", ["owned", "third_party"]).notNull(), independentSource: boolean("independentSource").default(false).notNull(), accessStatus: mysqlEnum("accessStatus", ["accessible", "inaccessible", "redirected", "blocked", "unknown"]).default("unknown").notNull(),
  firstCheckedAt: timestamp("firstCheckedAt"), lastCheckedAt: timestamp("lastCheckedAt"), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => ({ idProjectUnique: uniqueIndex("trust_evidence_sources_id_project_unique").on(table.id,table.projectId), projectUrlUnique: uniqueIndex("trust_evidence_sources_project_url_unique").on(table.projectId,table.sourceUrl), projectFk: foreignKey({columns:[table.projectId],foreignColumns:[projects.id],name:"trust_evidence_sources_project_fk"}) }));

export const trustSourceSnapshots = mysqlTable("trust_source_snapshots", {
  id: varchar("id", { length: 36 }).primaryKey(), projectId: int("projectId").notNull(), sourceId: varchar("sourceId", { length: 36 }).notNull(), snapshotVersion: int("snapshotVersion").notNull(), sourceUrlSnapshot: varchar("sourceUrlSnapshot", { length: 2000 }).notNull(),
  statusCode: int("statusCode"), accessedAt: timestamp("accessedAt").notNull(), contentHash: varchar("contentHash", { length: 128 }), titleSnapshot: varchar("titleSnapshot", { length: 500 }), ownerSnapshot: varchar("ownerSnapshot", { length: 255 }), canonicalUrl: varchar("canonicalUrl", { length: 2000 }), redirectUrl: varchar("redirectUrl", { length: 2000 }),
  robotsStatus: mysqlEnum("robotsStatus", ["allowed", "disallowed", "unknown", "not_checked"]).default("not_checked").notNull(), publicationTime: timestamp("publicationTime"), updatedTime: timestamp("updatedTime"), contentExcerpt: text("contentExcerpt"), metadata: json("metadata").$type<Record<string,unknown>|null>(), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => ({ idProjectUnique: uniqueIndex("trust_source_snapshots_id_project_unique").on(table.id,table.projectId), sourceVersionUnique: uniqueIndex("trust_source_snapshots_source_version_unique").on(table.sourceId,table.snapshotVersion), sourceProjectFk: foreignKey({columns:[table.sourceId,table.projectId],foreignColumns:[trustEvidenceSources.id,trustEvidenceSources.projectId],name:"trust_source_snapshots_source_project_fk"}) }));

export const trustEvidenceLedgerItems = mysqlTable("trust_evidence_ledger_items", {
  id: varchar("id", { length: 36 }).primaryKey(), evidenceId: varchar("evidenceId", { length: 64 }).notNull(), projectId: int("projectId").notNull(), sourceId: varchar("sourceId", { length: 36 }).notNull(), latestSnapshotId: varchar("latestSnapshotId", { length: 36 }),
  evidenceType: mysqlEnum("evidenceType", ["official_homepage","brand_definition","product_page","faq","help_center","team","company","organization_schema","brand_schema","product_schema","media","industry_platform","customer_case","partner","certification","industry_report","github","zhihu","wechat_official_account","interview","video","product_screenshot","service_process","verifiable_data","customer_review","demo"]).notNull(),
  reviewStatus: mysqlEnum("reviewStatus", ["unverified", "pending", "approved", "rejected"]).default("unverified").notNull(), confidence: int("confidence"), archivedAt: timestamp("archivedAt"), createdAt: timestamp("createdAt").defaultNow().notNull(), createdBy: int("createdBy"),
}, table => ({ idProjectUnique: uniqueIndex("trust_evidence_ledger_items_id_project_unique").on(table.id,table.projectId), projectEvidenceUnique: uniqueIndex("trust_evidence_ledger_items_project_evidence_unique").on(table.projectId,table.evidenceId), sourceProjectFk: foreignKey({columns:[table.sourceId,table.projectId],foreignColumns:[trustEvidenceSources.id,trustEvidenceSources.projectId],name:"trust_evidence_items_source_project_fk"}), snapshotProjectFk: foreignKey({columns:[table.latestSnapshotId,table.projectId],foreignColumns:[trustSourceSnapshots.id,trustSourceSnapshots.projectId],name:"trust_evidence_items_snapshot_project_fk"}) }));

export const trustEvidenceFactLinks = mysqlTable("trust_evidence_fact_links", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), evidenceItemId: varchar("evidenceItemId",{length:36}).notNull(), factId: int("factId").notNull(), relationship: mysqlEnum("relationship",["supports","contradicts","context_only"]).default("supports").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table=>({linkUnique:uniqueIndex("trust_evidence_fact_links_unique").on(table.evidenceItemId,table.factId,table.relationship),evidenceProjectFk:foreignKey({columns:[table.evidenceItemId,table.projectId],foreignColumns:[trustEvidenceLedgerItems.id,trustEvidenceLedgerItems.projectId],name:"trust_evidence_fact_links_item_project_fk"})}));

export const trustEvidenceQuestionLinks = mysqlTable("trust_evidence_question_links", {
  id: int("id").autoincrement().primaryKey(), projectId: int("projectId").notNull(), evidenceItemId: varchar("evidenceItemId",{length:36}).notNull(), questionVersionId: varchar("questionVersionId",{length:36}).notNull(), relationship: mysqlEnum("relationship",["primary","supporting","context_only"]).default("supporting").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table=>({linkUnique:uniqueIndex("trust_evidence_question_links_unique").on(table.evidenceItemId,table.questionVersionId),evidenceProjectFk:foreignKey({columns:[table.evidenceItemId,table.projectId],foreignColumns:[trustEvidenceLedgerItems.id,trustEvidenceLedgerItems.projectId],name:"trust_evidence_question_links_item_project_fk"}),questionProjectFk:foreignKey({columns:[table.questionVersionId,table.projectId],foreignColumns:[understandingQuestionVersions.id,understandingQuestionVersions.projectId],name:"trust_evidence_question_links_question_project_fk"})}));

export const trustEvidenceQualityChecks = mysqlTable("trust_evidence_quality_checks", {
  id: varchar("id",{length:36}).primaryKey(), projectId:int("projectId").notNull(), evidenceItemId:varchar("evidenceItemId",{length:36}).notNull(), snapshotId:varchar("snapshotId",{length:36}).notNull(),
  accessibility:mysqlEnum("accessibility",["pass","warning","fail","unknown"]).notNull(), authority:mysqlEnum("authority",["pass","warning","fail","unknown"]).notNull(), independence:mysqlEnum("independence",["pass","warning","fail","unknown"]).notNull(), consistency:mysqlEnum("consistency",["pass","warning","fail","unknown"]).notNull(), freshness:mysqlEnum("freshness",["pass","warning","fail","unknown"]).notNull(), relevance:mysqlEnum("relevance",["pass","warning","fail","unknown"]).notNull(),
  details:json("details").$type<Record<string,unknown>|null>(),checkedAt:timestamp("checkedAt").notNull(),createdAt:timestamp("createdAt").defaultNow().notNull(),
},table=>({itemSnapshotUnique:uniqueIndex("trust_evidence_quality_item_snapshot_unique").on(table.evidenceItemId,table.snapshotId),itemProjectFk:foreignKey({columns:[table.evidenceItemId,table.projectId],foreignColumns:[trustEvidenceLedgerItems.id,trustEvidenceLedgerItems.projectId],name:"trust_evidence_quality_item_project_fk"}),snapshotProjectFk:foreignKey({columns:[table.snapshotId,table.projectId],foreignColumns:[trustSourceSnapshots.id,trustSourceSnapshots.projectId],name:"trust_evidence_quality_snapshot_project_fk"})}));
