ALTER TABLE `brand_truth_profiles`
  ADD UNIQUE KEY `brand_truth_profiles_id_project_unique` (`id`,`projectId`);
--> statement-breakpoint
ALTER TABLE `brand_truth_fact_versions`
  ADD UNIQUE KEY `brand_truth_fact_versions_id_project_unique` (`id`,`projectId`);
--> statement-breakpoint
...
CREATE TABLE `brand_truth_profile_versions` (
  `id` varchar(36) NOT NULL, `projectId` int NOT NULL, `profileId` int NOT NULL, `version` int NOT NULL,
  `statusSnapshot` enum('draft','active','needs_review','archived') NOT NULL,
  `completenessScoreSnapshot` int NOT NULL, `verifiedFactRateSnapshot` int NOT NULL,
  `conflictCountSnapshot` int NOT NULL, `outdatedFactCountSnapshot` int NOT NULL,
  `lastReviewedAtSnapshot` timestamp NULL, `createdAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, `createdBy` int NULL,
  PRIMARY KEY (`id`), UNIQUE KEY `brand_truth_profile_versions_id_project_unique` (`id`,`projectId`),
  UNIQUE KEY `brand_truth_profile_versions_profile_version_unique` (`profileId`,`version`,`projectId`),
  CONSTRAINT `brand_truth_profile_versions_profile_project_fk` FOREIGN KEY (`profileId`,`projectId`) REFERENCES `brand_truth_profiles` (`id`,`projectId`)
);
--> statement-breakpoint
CREATE TABLE `brand_truth_profile_version_facts` (
  `id` int NOT NULL AUTO_INCREMENT, `projectId` int NOT NULL, `truthProfileVersionId` varchar(36) NOT NULL,
  `factVersionId` int NOT NULL, `createdAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `brand_truth_profile_version_fact_unique` (`truthProfileVersionId`,`factVersionId`),
  CONSTRAINT `brand_truth_profile_version_facts_profile_project_fk` FOREIGN KEY (`truthProfileVersionId`,`projectId`) REFERENCES `brand_truth_profile_versions` (`id`,`projectId`),
  CONSTRAINT `brand_truth_profile_version_facts_fact_project_fk` FOREIGN KEY (`factVersionId`,`projectId`) REFERENCES `brand_truth_fact_versions` (`id`,`projectId`)
);
--> statement-breakpoint
CREATE TABLE `understanding_assessments` (
  ...
  `extractionId` varchar(36) NOT NULL, `truthProfileVersionId` varchar(36) NOT NULL,
  ...
  UNIQUE KEY `understanding_assessments_extraction_governance_unique` (`extractionId`,`truthProfileVersionId`,`questionVersionId`,`extractionVersionId`,`methodologyVersionId`,`primaryRuleVersionId`),
  KEY `understanding_assessments_project_created_idx` (`projectId`,`createdAt`),
  CONSTRAINT `understanding_assessments_observation_extraction_project_fk` FOREIGN KEY (`extractionId`,`projectId`) REFERENCES `ai_observation_extractions` (`id`,`projectId`),
  CONSTRAINT `understanding_assessments_truth_profile_version_project_fk` FOREIGN KEY (`truthProfileVersionId`,`projectId`) REFERENCES `brand_truth_profile_versions` (`id`,`projectId`),
