...
CREATE TABLE `brand_truth_facts` (
	...
	`validFrom` timestamp,
	`validTo` timestamp,
	`temporalStatus` enum('current','historical','discontinued','unknown') DEFAULT 'unknown' NOT NULL,
	`companyEntityChange` boolean DEFAULT false NOT NULL,
	`productServiceStatus` enum('current','historical','discontinued','unknown') DEFAULT 'unknown' NOT NULL,
...
CREATE TABLE `understanding_assessment_dimension_results` (
	...
	`dimension` enum('identity','business','capability','boundary','temporal','evidence','consistency','uncertainty','category','products_services','customers','scenarios','capability_differentiation','boundary_temporal','product_service','target_customer','scenario') NOT NULL,
...
CREATE TABLE `understanding_assessment_manual_reviews` (
	...
	`projectId` int NOT NULL,
	`assessmentId` varchar(36) NOT NULL,
	`action` enum('confirmed','rejected','overridden','request_evidence','mark_insufficient_data') NOT NULL,
...
CREATE TABLE `understanding_question_versions` (
	...
	`locale` varchar(32) DEFAULT 'zh-CN' NOT NULL,
	`effectiveFrom` timestamp NOT NULL,
	`effectiveTo` timestamp,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`createdBy` int,
...
CREATE TABLE `understanding_methodology_dimension_definitions` (
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()), CONSTRAINT `understanding_methodology_dimension_definitions_id` PRIMARY KEY(`id`),
	CONSTRAINT `understanding_methodology_definition_dimension_unique` UNIQUE(`methodologyVersionId`,`dimension`),
...
CREATE TABLE `understanding_question_dimension_bindings` (
	...
	`secondaryDimensions` json NOT NULL, `subdimension` varchar(64), `createdAt` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `understanding_question_dimension_bindings_id` PRIMARY KEY(`id`),
	CONSTRAINT `understanding_question_dimension_question_unique` UNIQUE(`questionVersionId`),
...
CREATE TABLE `legacy_understanding_migration_runs` (
  `id` varchar(36) NOT NULL, `projectId` int NOT NULL, `mode` enum('dry_run','execute') NOT NULL,
  `migrationVersion` varchar(64) NOT NULL, `status` enum('running','completed','partially_completed','failed','cancelled') NOT NULL DEFAULT 'running',
  `resumeAfterLegacyEvaluationId` varchar(36), `scannedCount` int NOT NULL DEFAULT 0, `migratedCount` int NOT NULL DEFAULT 0,
  `partialCount` int NOT NULL DEFAULT 0, `skippedCount` int NOT NULL DEFAULT 0, `failedCount` int NOT NULL DEFAULT 0,
  `report` json, `startedAt` timestamp NOT NULL, `completedAt` timestamp NULL, `createdBy` int,
...
CREATE TABLE `legacy_understanding_migration_items` (
  `id` varchar(36) NOT NULL, `projectId` int NOT NULL, `migrationRunId` varchar(36), `legacyEvaluationId` varchar(36) NOT NULL,
  `sourceChecksum` varchar(71) NOT NULL, `migrationVersion` varchar(64) NOT NULL,
  ...
  `targetRunId` varchar(36), `targetAnswerId` varchar(36), `targetExtractionId` varchar(36), `targetAssessmentId` varchar(36),
  `missingFields` json, `legacyPayloadSnapshot` json, `failureReason` text, `migratedAt` timestamp NULL,
  `createdAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`id`),
...
CREATE TABLE `understanding_rollout_configs` (
  ...
  `writePath` enum('legacy','v2') NOT NULL DEFAULT 'legacy', `reason` text, `updatedBy` int,
