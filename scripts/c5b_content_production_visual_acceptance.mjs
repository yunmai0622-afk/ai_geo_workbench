/**
 * C5-B 内容资产生产页截图验收
 * 用法：pnpm dev 后 node scripts/c5b_content_production_visual_acceptance.mjs
 */
...
try {
  ...
  await page.getByText("AI 内容资产生产控制台").first().waitFor({ timeout: 30000 });
  await page.waitForTimeout(800);
  await page.screenshot({ path: resolve(ART, "c5b-content-production-desktop.png"), fullPage: true });
  console.log("[ok] c5b-content-production-desktop.png");

  for (const w of [375, 390, 414]) {
    await page.setViewportSize({ width: w, height: 812 });
    await page.goto(`${BASE}/weekly`, { waitUntil: "networkidle", timeout: 60000 });
    await page.getByText("内容资产生产").first().waitFor({ timeout: 30000 }).catch(() => {});
    await page.waitForTimeout(500);
    await assertNoHorizontalScroll(page, `mobile-${w}`);
    await page.screenshot({ path: resolve(ART, `c5b-content-production-mobile-${w}.png`), fullPage: true });
    console.log(`[ok] c5b-content-production-mobile-${w}.png`);
  }
} catch (e) {
  console.error(e);
  process.exit(1);
