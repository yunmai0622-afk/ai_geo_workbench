/**
 * C5-E AI 内容诊断页（目标问题区）截图验收
 * 用法：pnpm dev 后 node scripts/c5e_diagnosis_visual_acceptance.mjs
 */
...
try {
  ...
  await page.getByText("目标客户问题").first().waitFor({ timeout: 30000 });
  await page.waitForTimeout(800);
  await page.screenshot({ path: resolve(ART, "c5e-diagnosis-questions-desktop.png"), fullPage: true });
  console.log("[ok] c5e-diagnosis-questions-desktop.png");
  ...
  await page.screenshot({ path: resolve(ART, "c5e-diagnosis-questions-expanded.png"), fullPage: true });
  console.log("[ok] c5e-diagnosis-questions-expanded.png");
  ...
  for (const w of [375, 390, 414]) {
    ...
    await page.screenshot({ path: resolve(ART, `c5e-diagnosis-mobile-${w}.png`), fullPage: true });
    console.log(`[ok] c5e-diagnosis-mobile-${w}.png`);
  }
} catch (e) {
  console.error(e);
  process.exit(1);
