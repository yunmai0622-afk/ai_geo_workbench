/**
 * C5-C AI 内容诊断页截图验收
 * 用法：pnpm dev 后 node scripts/c5c_diagnosis_visual_acceptance.mjs
 */
...
try {
  ...
  await page.getByText("AI 内容诊断").first().waitFor({ timeout: 30000 });
  await page.getByText("核心诊断结论").first().waitFor({ timeout: 30000 });
  await page.waitForTimeout(800);
  await page.screenshot({ path: resolve(ART, "c5c-diagnosis-desktop.png"), fullPage: true });
  console.log("[ok] c5c-diagnosis-desktop.png");
  ...
  for (const w of [375, 390, 414]) {
    ...
    await page.screenshot({ path: resolve(ART, `c5c-diagnosis-mobile-${w}.png`), fullPage: true });
    console.log(`[ok] c5c-diagnosis-mobile-${w}.png`);
  }
} catch (e) {
  console.error(e);
  process.exit(1);
