...
describe("GEO V3.2 Brand Truth and Understand Engine acceptance", () => {
  ...
  it("adds immutable methodology and coverage snapshot fields without rewriting history", () => {
    ...
    expect(acceptanceMigration).toContain("MODIFY COLUMN IF EXISTS `severity` enum('P0','P1','P2') NULL");
    expect(acceptanceMigration).toContain("ADD COLUMN IF NOT EXISTS `methodologyVersion`");
