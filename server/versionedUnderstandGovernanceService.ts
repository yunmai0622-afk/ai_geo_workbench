...
export class VersionedUnderstandGovernanceService {
  ...
  async createAssessment(input: {
    ...
    const id = randomUUID();
    await this.db.transaction(async tx => {
      const [question, extractionVersion, methodology, primaryRule] = await Promise.all([
      ...
      if (!question[0] || !extractionVersion[0] || !methodology[0] || !primaryRule[0]) throw new Error("Assessment governance versions must belong to project");
