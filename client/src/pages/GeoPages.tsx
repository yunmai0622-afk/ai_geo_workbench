  ...
  if (isPublishing) {
    ...
    return (
      <PageShell title="内容发布" desc="内容发布页只展示可发布内容和已发布内容；第三方平台素材放入折叠区，当前只生成素材不自动登录发布；未授权或授权失效的平台会被阻断到人工发布前一步。" guide={guide}>
        ...
        <InfoCard title="第三方平台素材" desc="只生成素材，不自动登录发布" icon={FileText}>
          {selectedArticle ? <div className="space-y-4 text-sm leading-6 text-slate-300">
            <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4"><p className="font-semibold text-white">{selectedArticle.title}</p><p className="mt-1 text-slate-400">素材用于人工复制到官网、公众号、知乎、小红书、百家号/头条号等平台；系统不会保存第三方平台明文账号密码，也不会自动代表用户发布。</p></div>
