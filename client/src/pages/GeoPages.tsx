  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-gray-200 bg-white p-6 text-gray-900 shadow-[0_0_34px_rgba(56,189,248,0.10)] backdrop-blur">
        <p className="text-sm font-medium text-blue-600">{PLATFORM_PRODUCT_NAME}</p>
        ...
        <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <InfoCard title="去重后的内容缺口" desc="按问题语境合并展示" icon={AlertTriangle}>
            <div className="space-y-4 text-sm leading-6 text-gray-600">
              <div><p className="mb-2 font-medium text-white">内容缺口</p>{gapSummary.length ? <ul className="list-disc space-y-2 pl-5">{gapSummary.map(gap => <li key={gap}>{gap}</li>)}</ul> : <p className="text-gray-400">暂无明确缺口。</p>}</div>
              <div><p className="mb-2 font-medium text-white">优化建议</p>{suggestionSummary.length ? <ul className="list-disc space-y-2 pl-5">{suggestionSummary.map(item => <li key={item}>{item}</li>)}</ul> : <p className="text-gray-400">暂无建议。</p>}</div>
            </div>
          </InfoCard>
            ...
            <div className="space-y-3">
              {questionAnalyses.map(({ item, detail }, index) => (
                <div key={item.id ?? index} className="rounded-2xl border border-gray-200 bg-white p-4 text-sm leading-6 text-gray-600">
                  ...
                  <p className="mt-1 text-gray-400"><span className="text-gray-700">证据：</span>{textValue(detail.evidenceExcerpt, "原回答未提供足够可引用证据")}</p>
                  <p className="mt-1 text-gray-400"><span className="text-gray-700">竞品差距：</span>{textValue(detail.competitorGap, "未形成明确竞品差距")}</p>
                  <p className="mt-1 text-gray-400"><span className="text-gray-700">下一步：</span>{textValue(item.optimizationSuggestion)}</p>
                </div>
              ))}
            </div>
          </InfoCard>
      ...
      <PageShell title="内容发布" desc="内容发布页只展示可发布内容和已发布内容；第三方平台素材放入折叠区，当前只生成素材不自动登录发布；未授权或授权失效的平台会被阻断到人工发布前一步。" enabled={enabled} projectName={selectedProject?.enterpriseName} guide={guide}>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <InfoCard title="可发布内容" desc="通过准入检查" icon={Send}>
            <div className="space-y-3">
              ...
              {articleStatuses.filter(item => item.status === "允许发布").length === 0 ? <p className="text-sm text-gray-400">暂无可发布内容，请先在本周内容页完成文章、质量检查和人工审核。</p> : null}
            </div>
          </InfoCard>
          <InfoCard title="已发布内容" desc="进入监测链路" icon={RadioTower}>
            <div className="space-y-3">{(records.data ?? []).map(record => <div key={record.id} className="rounded-2xl border border-gray-200 bg-white/[0.04] p-4"><p className="font-semibold text-white">{record.publishChannel}</p><p className="mt-1 text-sm text-gray-600">{record.publishUrl} · {record.publishStatus}</p><p className="mt-1 text-xs text-gray-400">质量评分：{record.qualityScore ?? "未记录"}</p></div>)}{(records.data ?? []).length === 0 ? <p className="text-sm text-gray-400">暂无已发布内容。</p> : null}</div>
          </InfoCard>
        </div>
        <InfoCard title="第三方平台素材" desc="当前第三方平台暂不支持自动登录发布，只支持生成素材、复制发布、人工回填链接" icon={FileText}>
          {selectedArticle ? <div className="space-y-4 text-sm leading-6 text-gray-600">
            <div className="rounded-2xl border border-gray-200 bg-white p-4"><p className="font-semibold text-white">{selectedArticle.title}</p><p className="mt-1 text-gray-400">当前第三方平台暂不支持自动登录发布，只支持生成素材、复制发布、人工回填链接。素材用于人工复制到官网、公众号、知乎、小红书、百家号/头条号等平台；系统不会保存第三方平台明文账号密码，也不会自动代表用户发布。平台授权不在企业档案维护；复制与发布合规请在业务侧自行确认。</p></div>
            <div className="rounded-2xl border border-gray-200 bg-white/[0.03] p-4 text-sm text-gray-600">
              ...
              <p className="mt-2 text-gray-400">若历史数据中仍有平台登记信息，仅供参考；不阻断复制素材与人工发布流程。</p>
            </div>
