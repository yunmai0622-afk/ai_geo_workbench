...
function PageShell({
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-blue-100 bg-white p-6 text-gray-900 shadow-lg backdrop-blur">
  ...
  return (
    <PageShell title="内容诊断" desc="内容诊断页只展示客户问题、AI 回答、诊断结果、内容缺口、下一步建议五个区域，并保留一个主动作按钮。" enabled={enabled} projectName={selectedProject?.enterpriseName} guide={{ stage: "内容诊断", completion: a.length ? 58 : q.length ? 35 : 20, nextAction: next.label, why: "诊断结果决定内容写什么、补什么证据以及如何解释竞品差距。", risk: "样本量有限时不代表全网绝对排名。", ctaLabel: next.label, ctaPath: "/ai-diagnosis" }}>
      <div className="grid gap-4 lg:grid-cols-5">
        {[{ title: "客户问题", value: q.length, desc: q[0]?.questionText }, { title: "AI 回答", value: r.length, desc: r[0]?.aiPlatform }, { title: "诊断结果", value: a.length, desc: a[0]?.contentGap }, { title: "内容缺口", value: a.filter(item => item.contentGap).length, desc: a[0]?.optimizationSuggestion }, { title: "下一步建议", value: t.length, desc: t[0]?.taskName }].map(item => <div key={item.title} className="rounded-3xl border border-gray-200 bg-gray-50 p-5 text-gray-900"><p className="text-sm text-blue-600">{item.title}</p><p className="mt-2 text-3xl font-semibold text-gray-900">{item.value}</p><p className="mt-2 line-clamp-3 text-sm leading-6 text-gray-500">{textValue(item.desc, "暂无数据")}</p></div>)}
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <InfoCard title="去重后的内容缺口" desc="按问题语境合并展示" icon={AlertTriangle}>
            <div className="space-y-4 text-sm leading-6 text-gray-600">
              <div><p className="mb-2 font-medium text-white">内容缺口</p>{gapSummary.length ? <ul className="list-disc space-y-2 pl-5">{gapSummary.map(gap => <li key={gap}>{gap}</li>)}</ul> : <p className="text-gray-500">暂无明确缺口。</p>}</div>
              <div><p className="mb-2 font-medium text-white">优化建议</p>{suggestionSummary.length ? <ul className="list-disc space-y-2 pl-5">{suggestionSummary.map(item => <li key={item}>{item}</li>)}</ul> : <p className="text-gray-500">暂无建议。</p>}</div>
            </div>
          </InfoCard>
            ...
            <div className="space-y-3">
              {questionAnalyses.map(({ item, detail }, index) => (
                <div key={item.id ?? index} className="rounded-2xl border border-gray-200 bg-white p-4 text-sm leading-6 text-gray-600">
                  ...
                  <p className="mt-1 text-gray-500"><span className="text-gray-700">证据：</span>{textValue(detail.evidenceExcerpt, "原回答未提供足够可引用证据")}</p>
                  <p className="mt-1 text-gray-500"><span className="text-gray-700">竞品差距：</span>{textValue(detail.competitorGap, "未形成明确竞品差距")}</p>
                  <p className="mt-1 text-gray-500"><span className="text-gray-700">下一步：</span>{textValue(item.optimizationSuggestion)}</p>
                </div>
              ))}
            </div>
          </InfoCard>
    ...
    return (
      <PageShell title="内容发布" desc="内容发布页只展示可发布内容和已发布内容；第三方平台素材放入折叠区，当前只生成素材不自动登录发布；未授权或授权失效的平台会被阻断到人工发布前一步。" enabled={enabled} projectName={selectedProject?.enterpriseName} guide={guide}>
        <InfoCard title="企业资料" icon={Target}>
          <Button onClick={() => selectedProjectId && setLocation(buildProjectUrl("/enterprise-profile", selectedProjectId))} variant="outline" className="border-gray-200 text-gray-900 hover:bg-gray-50">进入企业档案</Button>
        </InfoCard>
            ...
            <div className="space-y-3">
              {articleStatuses.filter(item => item.status === "允许发布").map(item => (
                <div key={item.article.id} className="rounded-2xl border border-emerald-300/20 bg-emerald-400/10 p-4">
                  <p className="font-semibold text-gray-900">{item.article.title}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <Button onClick={() => setSelectedArticleId(item.article.id)} variant="outline" className="border-gray-200 text-gray-900 hover:bg-gray-50">查看第三方素材</Button>
                  </div>
                </div>
              ))}
              {articleStatuses.filter(item => item.status === "允许发布").length === 0 ? <p className="text-sm text-gray-500">暂无可发布内容，请先在本周内容页完成文章、质量检查和人工审核。</p> : null}
            </div>
          </InfoCard>
          <InfoCard title="已发布内容" desc="进入监测链路" icon={RadioTower}>
            <div className="space-y-3">{(records.data ?? []).map(record => <div key={record.id} className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><p className="font-semibold text-gray-900">{record.publishChannel}</p><p className="mt-1 text-sm text-gray-600">{record.publishUrl} · {record.publishStatus}</p><p className="mt-1 text-xs text-gray-500">质量评分：{record.qualityScore ?? "未记录"}</p></div>)}{(records.data ?? []).length === 0 ? <p className="text-sm text-gray-500">暂无已发布内容。</p> : null}</div>
          </InfoCard>
        </div>
        <InfoCard title="第三方平台素材" desc="当前第三方平台暂不支持自动登录发布，只支持生成素材、复制发布、人工回填链接" icon={FileText}>
          {selectedArticle ? <div className="space-y-4 text-sm leading-6 text-gray-600">
            <div className="rounded-2xl border border-gray-200 bg-white p-4"><p className="font-semibold text-gray-900">{selectedArticle.title}</p><p className="mt-1 text-gray-500">当前第三方平台暂不支持自动登录发布，只支持生成素材、复制发布、人工回填链接。素材用于人工复制到官网、公众号、知乎、小红书、百家号/头条号等平台；系统不会保存第三方平台明文账号密码，也不会自动代表用户发布。平台授权不在企业档案维护；复制与发布合规请在业务侧自行确认。</p></div>
      ...
      {selectedArticle ? <InfoCard title="文章正文详情" desc="正文、生成依据和素材可审阅" icon={FileText}>
        <div className="space-y-4 text-sm leading-6 text-gray-600">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-2xl border border-gray-200 bg-white/[0.03] p-4"><p className="font-medium text-white">引用友好片段</p><div className="mt-2 space-y-2">{selectedSnippets.map((snippet, index) => <div key={index} className="rounded-xl bg-white p-3"><p className="text-blue-700">{textValue(snippet.question, `片段 ${index + 1}`)}</p><p className="mt-1 text-gray-500">{textValue(snippet.answer)}</p></div>)}{selectedSnippets.length === 0 ? <p className="text-gray-500">暂无引用片段。</p> : null}</div></div>
...
export function MonitoringPage() {
  ...
  return (
    <PageShell title="收录监测" desc="收录监测页只展示已发布内容监测卡片，显示收录、AI 提及、AI 推荐、最近检测时间和当前建议。" enabled={enabled} projectName={selectedProject?.enterpriseName} guide={{ stage: "收录监测", completion: rows.length ? 88 : 74, nextAction: rows.length ? "查看监测结果并准备报告" : "先发布内容后再监测", why: "发布后的监测结果决定是否复测和优化。", risk: "不承诺保证收录、保证排名或保证被 AI 推荐。", ctaLabel: rows.length ? "进入交付报告" : "进入内容发布", ctaPath: rows.length ? "/delivery-reports" : "/content-publishing" }}>
      <div className="grid gap-4 lg:grid-cols-2">{rows.map(record => <div key={record.id} className="rounded-3xl border border-gray-200 bg-gray-50 p-5 text-gray-900"><RadioTower className="h-5 w-5 text-blue-600" /><h2 className="mt-3 font-semibold text-gray-900">{record.publishUrl}</h2><div className="mt-3 grid gap-2 text-sm text-gray-600 sm:grid-cols-2"><span>收录：待检测</span><span>AI 提及：待检测</span><span>AI 推荐：待检测</span><span>最近检测时间：{record.publishedAt ? new Date(record.publishedAt).toLocaleString() : "待检测"}</span></div><p className="mt-3 text-sm leading-6 text-gray-500">当前建议：{record.needRetest ? "进入复测，观察是否被收录、被 AI 提及、被 AI 推荐。" : "保持监测，等待更多样本。"}</p></div>)}{rows.length === 0 ? <p className="text-sm text-gray-500">暂无已发布内容监测卡片。</p> : null}</div>
    </PageShell>
  );
}
...
export function ReportsPage() {
  ...
  return (
    <PageShell title="交付报告" desc="交付报告页只展示 内容诊断报告、内容生产报告、发布监测报告、复测优化报告四类报告卡片，并保留风险说明。" enabled={enabled} projectName={selectedProject?.enterpriseName} guide={{ stage: "交付报告", completion: report.data ? 96 : 86, nextAction: report.data ? "查看并交付报告" : "生成交付报告", why: "报告把建档、诊断、内容、发布和监测串成客户可解释结果。", risk: "报告只能引用已确认事实，不承诺保证收录、排名或 AI 推荐。", ctaLabel: report.data ? "返回总览" : "生成交付报告", ctaPath: report.data ? "/" : "/delivery-reports" }}>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{reportTypes.map(type => <div key={type} className="rounded-3xl border border-gray-200 bg-gray-50 p-5 text-gray-900"><FileBarChart2 className="h-5 w-5 text-blue-600" /><h2 className="mt-3 font-semibold text-gray-900">{type}</h2><p className="mt-2 text-sm leading-6 text-gray-500">{report.data ? "已有最新报告数据，可用于客户交付。" : "等待生成最新报告。"}</p></div>)}</div>
