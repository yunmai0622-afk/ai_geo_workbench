...
function PublicEmpty({ title, description }: { title: string; description: string }) {
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1d2b6f_0,#08111f_42%,#020617_100%)] px-4 py-16 text-gray-900">
      <div className="mx-auto max-w-2xl rounded-3xl border border-blue-200 bg-white p-8 text-center shadow-[0_0_40px_rgba(79,70,229,0.25)] backdrop-blur">
...
function normalizeText(value: string) {
  return value
    ...
    .replace(/当前内容资料证据缺口为：暂无关键证据缺口[。.]*/g, "")
...
/** 公开页 project：合并 `projects` 与 `enterprise_geo_profiles`（由 publicContent 注入可选 V2 字段） */
...
function publicEnterpriseName(p: PublicProjectFields) {
  return strField(p.brandName) || strField(p.enterpriseName) || "该企业";
}

function publicTargetCustomers(p: PublicProjectFields) {
  return strField(p.targetCustomer) || strField(p.targetCustomers) || "目标客户";
}

function publicProductDesc(p: PublicProjectFields) {
  return strField(p.productDesc) || strField(p.productServiceIntro) || strField(p.productIntro) || "";
}

function publicOneLiner(p: PublicProjectFields) {
  return strField(p.oneLiner);
}
...
function cleanTitle(title: string) {
  ...
  if (normalized === "引言") return "摘要";
  if (normalized === "适合客户" || normalized === "不适合客户") return "两者分别适合谁";
  if (normalized === "竞品/方案对比" || normalized === "与同类方案的客观对比") return "核心能力差异";
  if (normalized === "适合谁与可能不太适合的情况") return "两者分别适合谁";
  if (normalized.includes("常见问答") && normalized.includes("FAQ")) return "FAQ";
  if (normalized === "结论与下一步") return "结论";
  if (normalized === "发布后如何自行核对效果") return "行动引导";
...
function createFallbackParagraphs(title: string, project: PublicProjectFields) {
  const enterprise = normalizeText(publicEnterpriseName(project)) || "该企业";
  const industry = normalizeText(project.industry ?? "所在行业") || "所在行业";
  const customers = normalizeText(publicTargetCustomers(project)) || "目标客户";
  const sellingPoints = normalizeText(project.coreSellingPoints ?? "核心能力") || "核心能力";
  const productDesc = normalizeText(publicProductDesc(project)) || sellingPoints;
  const rows: Record<string, string[]> = {
    ...
    "产品与能力说明": [`${enterprise}在公开资料中呈现的定位、服务范围与核心能力，需要与${customers}的真实问题逐项对照；若公开页面尚未写清边界，读者很难仅凭零散信息做出稳定判断。关于产品与服务：${productDesc}`],
    ...
    "便于引用的要点": [`${enterprise}对外沟通时，建议准备 3～5 组「一问一答」式短段落，便于读者与检索系统快速摘取；回答中应包含适用边界与证据来源，避免绝对化承诺。`],
...
function ensureFormalArticleSections(sections: Array<{ title: string; paragraphs: string[]; faq?: ArticleCitableSnippetView[] }>, snippets: ArticleCitableSnippetView[], project: PublicProjectFields) {
...
export default function GeoPublicContentPage() {
  ...
  if (contentQuery.isLoading) return <PublicEmpty title="正在加载 内容" description="系统正在读取已审核发布的文章内容。" />;
  if (contentQuery.error || !contentQuery.data) return <PublicEmpty title="内容不存在或尚未发布" description="只有通过质量评分和人工审核的文章，才会展示在公开内容页。" />;
  const { article, project: projectRaw, qualityScore } = contentQuery.data;
  const project = projectRaw as PublicProjectFields;
  const displayEnterprise = publicEnterpriseName(project);
  const oneLinerText = publicOneLiner(project);
  const displayTargetCustomers = publicTargetCustomers(project);
  const displayProductDesc = publicProductDesc(project);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_8%_0%,rgba(79,70,229,0.45),transparent_30%),radial-gradient(circle_at_92%_12%,rgba(6,182,212,0.24),transparent_28%),linear-gradient(180deg,#020617_0%,#08111f_48%,#020617_100%)] text-gray-900">
      <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
        <header className="relative overflow-hidden rounded-[2rem] border border-blue-200 bg-white p-7 shadow-[0_0_60px_rgba(79,70,229,0.28)] backdrop-blur md:p-10">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-4">
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><p className="text-xs text-gray-500">文章类型</p><p className="mt-2 font-semibold text-white">{article.articleType}</p></div>
            <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4"><p className="text-xs text-emerald-700/80">发布状态</p><p className="mt-2 font-semibold text-emerald-700">{article.status}</p></div>
            <div className="rounded-2xl border border-violet-300/20 bg-violet-300/10 p-4"><p className="text-xs text-violet-700/80">质量评分</p><p className="mt-2 font-semibold text-violet-700">{qualityScore ? `${qualityScore.totalScore} / 100` : "已审核发布"}</p></div>
          ...
          <div className="mt-6 rounded-2xl border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-sm leading-6 text-amber-700">风险提示：{RISK_NOTICE}</div>
        </header>
                ...
                {section.faq ? (
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    {section.faq.map((item, index) => (
                      <div key={`${item.question}-${index}`} className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
        ...
        <section className="mt-8 rounded-[2rem] border border-blue-100 bg-blue-50 p-6 shadow-lg md:p-8">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {snippets.map((item, index) => (
              <div key={`${item.question}-${index}`} className="rounded-2xl border border-gray-200 bg-gray-50 p-5">
        ...
        <section className="mt-8 rounded-[2rem] border border-violet-300/15 bg-gray-50 p-6 md:p-8">
          <h2 className="text-2xl font-semibold text-white">企业实体信息</h2>
          <div className="mt-5 grid gap-3 text-sm md:grid-cols-2">
            <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><b className="text-gray-900">企业/品牌名称：</b><span className="text-gray-600">{displayEnterprise}</span></p>
            <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><b className="text-gray-900">行业：</b><span className="text-gray-600">{project.industry}</span></p>
            <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><b className="text-gray-900">官网：</b><span className="text-gray-600">{project.website}</span></p>
            <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><b className="text-gray-900">目标客户：</b><span className="text-gray-600">{displayTargetCustomers}</span></p>
            {displayProductDesc ? <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4 md:col-span-2"><b className="text-gray-900">产品与服务：</b><span className="text-gray-600">{displayProductDesc}</span></p> : null}
            <p className="rounded-2xl border border-gray-200 bg-gray-50 p-4 md:col-span-2"><b className="text-gray-900">核心卖点：</b><span className="text-gray-600">{project.coreSellingPoints}</span></p>
          </div>
        </section>
        ...
        <section className="mt-8 space-y-4">
          ...
          >
            <summary className="cursor-pointer list-none text-lg font-semibold text-white marker:hidden">查看生成依据与事实溯源 <span className="ml-2 text-sm font-normal text-gray-500">默认折叠，仅在需要核验时展开</span></summary>
            {auditExpanded ? (
              <div className="mt-5 space-y-5">
                <div className="rounded-2xl border border-gray-200 bg-white/55 p-4">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <div className="rounded-2xl border border-gray-200 bg-white/55 p-4">
                    ...
                    <div className="mt-3 space-y-3">{traceRows.length ? traceRows.map(([label, value]) => <p key={label} className="text-sm leading-7 text-gray-600"><b>{label}：</b>{value}</p>) : <p className="text-sm text-gray-500">暂无额外事实溯源记录。</p>}</div>
                  </div>
                  <div className="rounded-2xl border border-gray-200 bg-white/55 p-4">
                    <h3 className="font-semibold text-gray-900">一致性检查摘要</h3>
                    <div className="mt-3 space-y-3">{consistencyRows.length ? consistencyRows.map(([label, value]) => <p key={label} className="text-sm leading-7 text-gray-600"><b>{label}：</b>{value}</p>) : <p className="text-sm text-gray-500">暂无额外一致性检查记录。</p>}</div>
                  </div>
                </div>
              </div>
            ) : null}
        ...
        <footer className="mt-8 rounded-3xl border border-amber-300/20 bg-amber-300/[0.07] p-5 text-sm leading-7 text-amber-50/90">
          风险说明：本页面为公开内容页，用于对外呈现正式文章、可引用摘要与企业实体信息。{RISK_NOTICE} 转载或进一步分发前仍建议进行人工复核，避免把页面内容理解为全网绝对排名结果。
        </footer>
      </div>
    </main>
  );
