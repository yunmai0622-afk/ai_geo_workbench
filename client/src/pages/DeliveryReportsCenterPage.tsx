          ...
          selectedProjectId === 210001 ? (
            ...
            >
              ...
              <div className="mt-3 rounded-xl border border-sky-100 bg-white/80 p-3 text-xs leading-5 text-gray-700" data-testid="delivery-report-automatic-retest-status">
                ...
                <p className={scheduledRetestQuery.data?.modelChannels.independent ? "text-emerald-800" : "font-medium text-amber-800"}>{scheduledRetestQuery.data?.modelChannels.independent ? "豆包与 DeepSeek 独立模型通道配置已验证。" : "已完成多通道 AI 复测，独立模型通道配置待完善。"}</p>
                ...
                {scheduledRetestQuery.data?.platformResults.map(platform => <p key={platform.engine}>{scheduledRetestQuery.data?.modelChannels.independent ? "独立模型通道" : "逻辑通道"} {platform.engineName}：{platform.resultCount}/{platform.expectedCount} 条，提及 {platform.mentionCount}，推荐 {platform.recommendCount}，知乎引用 {platform.citedSampleUrlCount}，{platform.status === "success" ? "成功" : platform.status === "partial" ? "部分失败" : "失败"}。</p>)}
  ...
  return (
    <div className="space-y-6 pb-12" data-testid="delivery-report-page">
      ...
      {selectedProjectId ? <section className="rounded-2xl border border-fuchsia-100 bg-fuchsia-50/40 p-5 shadow-sm" data-testid="delivery-report-understanding-change">
        ...
        <div className="mt-4 rounded-xl border border-fuchsia-100 bg-white p-4 text-sm leading-6 text-gray-700"><p><span className="font-semibold">本次基线准确项：</span>{(understandingQuery.data?.evaluations ?? []).some(item => ["accurate", "mostly_accurate"].includes(item.finalStatus)) ? "已有部分回答与核验事实一致；这是首次基线，不代表已经改善。" : "暂无足够准确项证据。"}</p><p><span className="font-semibold">偏差与待复核：</span>{(understandingQuery.data?.severityCounts.P0 ?? 0) + (understandingQuery.data?.severityCounts.P1 ?? 0)} 项 P0/P1 待处理或待复核。</p><p><span className="font-semibold">尚无法判断：</span>{understandingQuery.data?.dataSufficient ? "8 维基线数据已具备；是否改善仍需下一轮同题复测。" : "理解维度数据不完整，不形成增长结论。"}</p><p><span className="font-semibold">下一轮：</span>完成事实核验和非文章纠偏动作后，使用固定 Understand 问题集再次验证。</p></div>
      </section> : null}
