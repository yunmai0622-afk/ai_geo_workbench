...
const toneClasses = {
  ...
  violet: "border-violet-300/20 bg-violet-400/10 text-violet-700 shadow-violet-500/10",
  emerald: "border-emerald-300/20 bg-emerald-400/10 text-emerald-700 shadow-emerald-500/10",
  amber: "border-amber-300/20 bg-amber-400/10 text-amber-700 shadow-amber-500/10",
...
function Section({ eyebrow, title, description, children }: SectionProps) {
  return (
    <section className="scroll-mt-28 rounded-[2rem] border border-gray-200 bg-white/55 p-5 shadow-2xl shadow-blue-950/20 backdrop-blur md:p-7">
...
function ReadOnlyNotice() {
  return (
    <div className="rounded-3xl border border-amber-300/20 bg-amber-400/10 p-5 text-sm leading-6 text-amber-50 shadow-xl shadow-amber-950/10">
      ...
      <p className="mt-2 text-amber-700/80">本页面不提供登录、编辑、生成、发布、删除、保存或更新状态能力；展示内容均为“海豚知道”样板项目脱敏数据。</p>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#040816] text-gray-900">
      <div className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute left-[-12%] top-[-18%] h-[32rem] w-[32rem] rounded-full bg-blue-100/50 blur-3xl" />
      ...
      <header className="sticky top-0 z-20 border-b border-gray-200 bg-[#040816]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between md:px-6">
          <a href="#overview" className="group inline-flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-blue-200 bg-blue-50 text-sm font-bold text-blue-700 shadow-lg shadow-blue-200">内容增长系统</span>
            <span>
              <span className="block text-sm font-semibold text-white">V1.2 外部只读 Demo</span>
              <span className="block text-xs text-gray-500">{demoProject.shortName} 样板项目</span>
            </span>
          </a>
          <nav className="flex gap-2 overflow-x-auto text-xs text-gray-600 md:text-sm">
            {[
              ...
              <a key={id} href={`#${id}`} className="rounded-full border border-gray-200 bg-gray-50 px-3 py-2 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">
                {label}
              </a>
            ))}
          </nav>
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 md:px-6 md:py-12">
        <section id="overview" className="relative overflow-hidden rounded-[2.5rem] border border-blue-200 bg-gradient-to-br from-slate-950 via-blue-950/55 to-violet-950/60 p-6 shadow-2xl shadow-cyan-950/30 md:p-10">
          <div className="absolute right-8 top-8 hidden h-40 w-40 rounded-full border border-blue-100 bg-blue-50 blur-sm md:block" />
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
            <div className="rounded-3xl border border-gray-200 bg-gray-50 p-5">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
                {growthPath.map((step, index) => (
                  <div key={step} className="relative rounded-2xl border border-blue-100 bg-white p-4">
            ...
            <div className="space-y-4">
              <div className="rounded-3xl border border-emerald-300/15 bg-emerald-400/5 p-5">
                <p className="text-sm font-semibold text-emerald-700">AI 今日建议</p>
              ...
              <div className="rounded-3xl border border-amber-300/15 bg-amber-400/5 p-5">
                <p className="text-sm font-semibold text-amber-700">待处理任务</p>
        ...
        <Section eyebrow="02 / Assets" title="企业资产" description="只读展示海豚知道样板项目的基础资料、产品服务、案例采集、竞品、合规、风格和发布策略，不展示真实客户敏感数据。">
          <div id="assets" className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {assetSections.map((section) => (
              <article key={section.title} className="rounded-3xl border border-gray-200 bg-gray-50 p-5 shadow-xl shadow-blue-950/10">
            ...
            {diagnosisQuestions.map((item, index) => (
              <article key={item.question} className="rounded-3xl border border-gray-200 bg-white p-5">
                ...
                <div className="mt-4 grid gap-3 text-xs leading-5 text-gray-600 md:grid-cols-2">
                  <p><span className="text-violet-600">语义分析：</span>{item.analysis}</p>
        ...
        <Section eyebrow="04 / Content" title="内容生产" description="展示 3 篇核心 内容，每篇包含 8 项生成依据、事实溯源表、质量评分、一致性检查、发布前检查和 AI 可引用片段。">
          <div id="content" className="space-y-5">
            {demoArticles.map((article) => (
              <article key={article.title} className="rounded-[2rem] border border-gray-200 bg-gray-50 p-5 shadow-xl shadow-violet-950/10 md:p-6">
                ...
                <div className="mt-5 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <p className="rounded-2xl border border-emerald-300/15 bg-emerald-400/5 p-4 text-sm leading-6 text-gray-600"><span className="text-emerald-700">一致性检查：</span>{article.consistencyCheck}</p>
                ...
                <div className="mt-5 rounded-3xl border border-violet-300/15 bg-violet-400/5 p-4">
                  <h4 className="font-semibold text-violet-700">AI 可引用片段</h4>
                  <ul className="mt-3 space-y-2 text-sm leading-6 text-gray-600">
          ...
          <div id="publish" className="space-y-4">
            {publishRecords.map((record) => (
              <article key={record.title} className="rounded-3xl border border-gray-200 bg-white p-5">
                ...
                <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-gray-600">
        ...
        <Section eyebrow="06 / Monitoring" title="收录监测" description="展示发布后监测记录、收录状态、AI 提及状态、AI 推荐状态、当前建议和未达成时的优化建议。">
          <div id="monitoring" className="grid gap-4 lg:grid-cols-2">
            {monitoringRecords.map((record) => (
              <article key={record.target} className="rounded-3xl border border-gray-200 bg-gray-50 p-5">
        ...
        <Section eyebrow="07 / Report" title="报告中心" description="展示至少 1 份客户可读 内容增长系统 试跑报告，并保留样本量有限的风险说明。">
          <div id="report" className="grid gap-4 lg:grid-cols-[1fr_0.85fr]">
            ...
            <div className="rounded-3xl border border-amber-300/20 bg-amber-400/10 p-5 shadow-xl shadow-amber-950/10">
              <p className="text-sm font-semibold text-amber-700">风险提示卡</p>
              <p className="mt-3 text-sm leading-6 text-amber-50/90">{reportSummary.risk}</p>
              <p className="mt-3 text-xs leading-5 text-amber-700/75">Demo 中的监测状态为样板展示和待人工复测口径，不能用于对客户作效果保证。</p>
            </div>
          </div>
        </Section>

        <section className="rounded-[2rem] border border-gray-200 bg-gray-50 p-5 md:p-7">
