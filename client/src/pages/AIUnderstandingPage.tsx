  ...
  return <div className="space-y-6 pb-12" data-testid="ai-understanding-page">
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6" data-testid="understanding-dimensions">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {data.dimensionResults.map(dimension => <article key={dimension.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4" data-testid={`understanding-dimension-${dimension.id}`}><div className="flex items-start justify-between gap-2"><h3 className="font-medium text-gray-900">{dimension.label}</h3><span className="text-xs text-gray-400">权重 {dimension.weight}%</span></div><p className="mt-4 text-2xl font-bold tabular-nums text-gray-950">{dimension.score == null ? "—" : dimension.score}</p><div className="mt-3 flex flex-wrap items-center gap-2"><span className={`inline-flex rounded-full border px-2 py-0.5 text-xs ${statusClass(dimension.status)}`}>{UNDERSTANDING_STATUS_LABELS[dimension.status as UnderstandingFieldStatus] ?? "暂无法核验"}</span>{dimension.severity ? <span className="text-xs font-semibold text-red-700">{dimension.severity}</span> : null}</div><details className="mt-3 text-xs text-gray-600"><summary className="cursor-pointer font-medium text-gray-700">查看事实、AI 表达与证据</summary><div className="mt-2 space-y-1 leading-5"><p>标准事实：{dimension.expectedFacts.length ? dimension.expectedFacts.map(item => String(item.value ?? "")).filter(Boolean).join("；") : "数据不足"}</p><p>AI 实际表达：{dimension.actualStatements.length ? dimension.actualStatements.join("；") : "未覆盖"}</p><p>匹配：{dimension.matchedFacts.join("、") || "无"}；缺失：{dimension.missingFacts.join("、") || "无"}</p><p>错误：{dimension.inaccurateFacts.join("、") || "无"}；过时：{dimension.outdatedFacts.join("、") || "无"}</p><p>暂无法核验：{dimension.unverifiableClaims.join("、") || "无"}；证据 ID：{dimension.evidenceReferences.join("、") || "无"}</p><p>建议：{dimension.recommendedCorrection || "继续保留固定问题复测"}</p></div></details></article>)}
      </div>
