            ...
            <div className="flex flex-col gap-2">
              {painPresets.map(p => (
                <label key={p} className="flex cursor-pointer items-center gap-2 text-gray-700">
                  <input type="checkbox" checked={props.customerPains.includes(p)} onChange={e => props.onTogglePain(p, e.target.checked)} className="h-4 w-4 rounded border-gray-300 bg-white text-blue-600" />
                  {p}
                </label>
              ))}
            </div>
